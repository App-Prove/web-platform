...
function AnimatedMagicCard({ children, delay = 0 }: { children: React.ReactNode; delay?: number }) {
...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="min-h-full px-2 sm:px-0 mb-24 w-full flex flex-col sm:gap-30 overflow-hidden sm:overflow-visible">
      <section className="text-center mt-32 sm:mt-24 flex relative flex-col gap-4">
        <motion.h1 
          className="text-4xl font-medium z-10"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          N&apos;attendez plus pour sécuriser votre code
        </motion.h1>
        <motion.p 
          className="text-lg text-muted-foreground z-10 text-center sm:px-0"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.2 }}
        >
          Scan de votre codebase, identification des failles de sécurité et résolutions des bugs en quelques clics.
        </motion.p>
        <motion.div 
          className="sm:mt-8 z-10 flex justify-center"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.div
          className="relative w-fit h-[500px] overflow-hidden -translate-y-52 pointer-events-none transition-opacity duration-300"
          initial={{ opacity: 0, y: 50 }}
      ...
      >
        <section className="flex flex-col gap-16 -mt-60 sm:mt-32 z-10" id="incubators">
          <motion.h2 
            ...
            transition={{ duration: 0.6 }}
          >
            Soutenu par des incubateurs de renommée internationale
          </motion.h2>
          <motion.div 
        ...
        <section className="flex flex-col gap-8 py-24" id="features">
          ...
          <div className="flex flex-col sm:flex-row gap-4 h-fit sm:h-48">
            <AnimatedMagicCard delay={0.1}>
              Scan automatique de votre codebase
            </AnimatedMagicCard>
            <AnimatedMagicCard delay={0.2}>
              Identification des failles de sécurité
            </AnimatedMagicCard>
            <AnimatedMagicCard delay={0.3}>
